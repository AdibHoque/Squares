...
member.timeout(null, `${reason} Moderator: ${interaction.user.username}#${interaction.user.discriminator}`).then(() => {
    ...
    const embed = new EmbedBuilder().setTitle(`You were unmuted in ${interaction.guild.name}`).setDescription(`**Reason:** `+reason).setColor("FF9900").setFooter({text: date})
    ...
    l.findOne({ GuildID: guildid, UserID: userid }, async (err, data) => {
        ...
        g.findOne({GuildID: guildid}, async (err, data) => {
            ...
            c.send({embeds:[logEmbed("MEMBER UNMUTED", usertag, userid, modtag, modid, reason, uid)]})
        })
    })
}).catch(error => {
     console.log(error);
