...
.addIntegerOption(option => option.setName("amount").setDescription("Amount to delete. Default and Maximum both is 25."))
...
l.findOne({GuildID: guildid, UserID: userid}, async (err, data) => {
    if(err) throw err;
    if(!data) return interaction.editReply({embeds:[errorEmbed("**This member has no logs.**")]});
    if(!data.Content) return interaction.editReply({embeds:[errorEmbed("**This member has no logs..**")]});
    // interaction.editReply for confirmation
    // message collector
    const embed = new EmbedBuilder().setDescription("Please type in `CONFIRM` to proceed this action, as it cannot be undone.\nThis command will be automatically cancelled in 20 seconds.").setColor("#FF9900")
    interaction.editReply({embeds:[embed]})
    const filter = m => m.author.id==interaction.user.id;
    const collector = interaction.channel.createMessageCollector({filter, time: 20*1000 });
...
collector.on('collect', async m => {
    if(m.content == "CONFIRM") {
     ...
     data.Content = arr
     data.save()
     return interaction.editReply({embeds:[successEmbed(`**Corresponding amount of starting logs were cleared from ${member.user.tag}.**`)]});
    }
})
