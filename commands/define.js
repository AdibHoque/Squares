...
module.exports.data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      .setName("word")
      .setDescription("The word or words you wanna define.")
      .setRequired(true)
  );
...
module.exports.run = async (client, interaction, options) => {
  ...
  body.list.forEach((d) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .setURL(d.permalink)
      .addFields([
        { name: "Definition", value: "```" + def.substr(0, 1000) + "```" },
        { name: "Example", value: "```" + exa.substr(0, 1000) + "```" },
      ])
      .setFooter({
        text:
          `Page ${body.list.indexOf(d) + 1} of ${body.list.length} - ⬆️ ` +
          d.thumbs_up +
          ` | ` +
          `⬇️ ` +
          d.thumbs_down,
  ...
  const row = new ActionRowBuilder().addComponents([
    new ButtonBuilder()
      .setEmoji("1062331837290119269")
      .setStyle(1)
      .setCustomId(`define-first`)
      .setDisabled(true),
    new ButtonBuilder()
      .setEmoji("1062331801395286116")
      .setStyle(1)
      .setCustomId(`define-previous`),
    new ButtonBuilder()
      .setEmoji("1062331764003053578")
      .setStyle(4)
      .setCustomId(`define-delete`),
    new ButtonBuilder()
      .setEmoji("1062331693429694545")
      .setStyle(1)
      .setCustomId(`define-next`),
    new ButtonBuilder()
      .setEmoji("1062331656859566192")
      .setStyle(1)
      .setCustomId(`define-last`),
  ]);

  interaction.editReply({ embeds: [embed], components: [row] });

  const filter = (i) =>
    i.customId.startsWith("define") && i.user.id === interaction.user.id;
