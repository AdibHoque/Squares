...
async function start(
  ...
  collector.on("collect", async (m) => {
    ...
    if (m.content.length > 0 && m.content.length < minletters) {
      const embed = new EmbedBuilder()
        .setDescription(
          `__**${m.content.toUpperCase()}**__ is incorrect.\n**Reason:** ${
            m.content.length
          } Characters.`
        )
        .addFields([
          {
            name: "Criteria:",
            value: `Minimum Characters: **${minletters}**\nStarting Letter: **${startletter}**`,
          },
          {
            name: "Time Left:",
    ...
    if (
      m.content &&
      !m.content.toLowerCase().startsWith(startletter.toLowerCase())
    ) {
      const embed = new EmbedBuilder()
        .setDescription(
    ...
    if (
      ...
      if (w == true) {
        ...
        if (words.length > longestword.length)
          await db.set(`wc${interaction.channelId}.longestword`, words);
    ...
    if (reason && reason === "time") {
      if (players.length == 2) {
        ...
        const embed = new EmbedBuilder()
          .setTitle("Time Out!")
          .setDescription(
            `<@${currentplayer}> couldn't send a correct word within time.`
          )
          .addFields([
            { name: "Eliminated Player:", value: `<@${currentplayer}>` },
            { name: "Next Player:", value: `<@${nextplayer}>` },
          ])
          .setColor("#FF0000");
...
module.exports.run = async (client, interaction, options) => {
  ...
  await db.set(`wc${interaction.channelId}`, {
    lastUpdate: Date.now(),
    usedWords: [],
    longestword: "word",
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `<@${interaction.user.id}> is starting a Wordchain game!\nYou may join to start playing together.`
    )
    .addFields([
    ...
    if (!players.includes(i.user.id) && !i.user.bot && players.length < 11) {
      ...
      const row = new ActionRowBuilder().addComponents([
        new ButtonBuilder()
          ...
          .setCustomId(`join`),
        new ButtonBuilder()
          .setLabel(`Participants: ${players.length}`)
          .setStyle(2)
          .setCustomId(`p`)
          .setDisabled(true),
      ]);
  ...
  collector.on("end", async (collected) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields([{ name: "Participants:", value: p.join("\n") }])
      .setColor("#F3BA2F");
