...
module.exports.help = {
  ...
  category: "Utility",
  description:
    "Calculate how many wins you need for reaching a certain Winrate.",
  required: "None",
...
module.exports.data = new SlashCommandBuilder()
  ...
  .addNumberOption((option) =>
    option
      .setName("current_winrate")
      .setDescription("Your current winrate.")
      .setRequired(true)
  )
  .addIntegerOption((option) =>
    option
      .setName("number_of_matches")
      .setDescription("Number of matches you played.")
      .setRequired(true)
  )
  .addNumberOption((option) =>
    option
      .setName("desired_winrate")
      .setDescription("The Winrate you wanna reach.")
      .setRequired(true)
  );
...
module.exports.run = async (client, interaction, options) => {
  ...
  const result = Math.round(
    (matches * (desiredwr - currentwr)) / (100 - desiredwr)
  );

  const embed = new EmbedBuilder()
    .setTitle("Winrate Calculator")
    .setDescription(
      `${currentwr} Winrate >> ${desiredwr}% Winrate (${matches} Matches)`
    )
